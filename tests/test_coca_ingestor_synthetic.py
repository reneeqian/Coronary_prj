...
def test_dataset_structure_validation(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="DAT-001: missing patient/ directory raises DatasetStructureError"
    )
    ...
    if not raised:
        report.error(
            "Expected DatasetStructureError when patient/ dir is absent; none raised", "DAT-001"
        )
    report.info(
        "DatasetStructureError raised when dataset root has no patient/ directory", "DAT-001"
    )
...
def test_slices_sorted_by_z(tmp_path, request, evidence_output_dir):
    ...
    report.info("Slices correctly sorted by z-position", requirement_tag="DAT-004")
...
def test_hounsfield_rescale_applied(tmp_path, request, evidence_output_dir):
    ...
    report.info("Hounsfield rescale applied correctly", requirement_tag="DAT-004")
...
def test_annotation_out_of_bounds_raises(tmp_path, request, evidence_output_dir):
    ...
    report.info("Out-of-bounds annotation correctly rejected", requirement_tag="DAT-004")
...
def test_invalid_patient_id_raises(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="DAT-003: patient directory with no DICOM series raises DatasetStructureError"
    )
    ...
    if not raised:
        report.error(
            "Expected DatasetStructureError for patient dir with no series subdirs; none raised",
            "DAT-003",
        )
    report.info(
        "DatasetStructureError raised for patient directory with no series subdirectories",
        "DAT-003",
    )
...
def test_lazy_patient_loading(tmp_path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DAT-006: COCAGatedIngestor loads each patient volume exactly once"
    )
    ...
    with patch.object(
        COCAGatedIngestor,
        "_load_image_volume",
        return_value=(np.zeros((1, 2, 2)), (1.0, 1.0, 1.0), {}),
    ) as mock_loader:
        ingestor = COCAGatedIngestor(dataset_root=tmp_path)
        ingestor.ingest_patient("0")
        report.info(
            f"_load_image_volume called {mock_loader.call_count} time(s) for 1 patient ingestion",
            "DAT-006",
        )
...
def test_slice_index_out_of_bounds(tmp_path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DAT-007: annotation referencing out-of-bounds slice is silently ignored"
    )
    ...
    report.info(
        f"vector_rois={sample.annotations.vector_rois!r} (expected None — out-of-bounds annotation ignored)",
        "DAT-007",
    )
...
def test_deterministic_slice_retrieval(tmp_path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DAT-008: loading the same patient twice returns identical slice data"
    )
...
def test_missing_dicom_files(tmp_path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DAT-005: patient series dir with no DICOM files raises DatasetStructureError"
    )
    ...
    report.info(
        "DatasetStructureError raised when patient series directory has no .dcm files", "DAT-005"
    )
...
def test_get_patient_api(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="DAT-006: load_patient_sample returns PatientSample with correct patient_id"
    )
    patient_dir = tmp_path / "patient" / "0" / "seriesA"
    patient_dir.mkdir(parents=True)
    ...
    report.info(
        f"load_patient_sample returned PatientSample with patient_id='{patient.patient_id}'",
        "DAT-006",
    )
...
def test_get_volume_api(tmp_path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DAT-006: load_patient_sample returns a volume with expected shape"
    )
...
def test_ingest_dataset_enumerates_all_patient_ids(tmp_path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DAT-011: ingest_dataset enumerates all patient IDs without loading volumes"
    )
...
def test_annotation_with_missing_image_index(tmp_path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DAT-009: annotation entry missing ImageIndex is treated as no annotation"
    )
    ...
    report.info(
        f"vector_rois={sample.annotations.vector_rois!r} (expected None — entry without ImageIndex ignored)",
        "DAT-009",
    )
...
def test_ct_volume_sorted_by_z_position(tmp_path, evidence_output_dir):
    ...
    if not np.all(volume[0] == 0) or not np.all(volume[1] == 10):
        report.error(
            f"Volume not sorted by Z: slice0={volume[0, 0, 0]}, slice1={volume[1, 0, 0]}", "DAT-012"
        )
    report.info(
        f"Slices sorted by Z: slice0 pixel={volume[0, 0, 0]:.0f} (z=0), slice1 pixel={volume[1, 0, 0]:.0f} (z=10)",
        "DAT-012",
    )
...
def test_get_sample_generates_image_and_mask(tmp_path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DAT-004: get_sample returns (image, mask) arrays with non-zero mask for annotated patient"
    )
    ...
    report.info(
        f"get_sample: X.shape={X.shape}, Y.shape={Y.shape}, np.sum(Y)={np.sum(Y):.0f}", "DAT-004"
    )
...
def test_get_sample_returns_single_annotated_slice(tmp_path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DAT-006: get_sample returns exactly one image/mask pair for a single-slice annotated patient"
    )
...
def test_get_sample_no_annotations_returns_empty(tmp_path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DAT-004: get_sample returns empty arrays when patient has no annotations"
    )
    ...
    report.info(
        f"No annotations: X.shape[0]={X.shape[0]}, Y.shape[0]={Y.shape[0]} (both expected 0)",
        "DAT-004",
    )
...
def test_get_sample_multiple_rois_same_slice(tmp_path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DAT-004: multiple ROIs on the same slice are merged into one mask"
    )
    ...
    report.info(
        f"Two ROIs merged: X.shape[0]={X.shape[0]}, np.sum(Y)={np.sum(Y):.0f} (expected >2)",
        "DAT-004",
    )
...
def test_get_sample_skips_invalid_slice_annotations(tmp_path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DAT-007: annotation with out-of-bounds ImageIndex is skipped; get_sample returns empty arrays"
    )
    ...
    plist_data = {"Images": [{"ImageIndex": 10, "ROIs": []}]}
    ...
    report.info(
        f"Out-of-bounds annotation skipped: X.shape[0]={X.shape[0]}, Y.shape[0]={Y.shape[0]} (both expected 0)",
        "DAT-007",
    )
