...
def _write_training_run(root, name="20260427_100000_000000"):
    ...
    data = {
        ...
        "config": {"learning_rate": 0.001, "epochs": 50, "batch_size": 8,
                   "device": "cpu", "optimizer": "Adam", "task": "CoronaryCalciumTask"},
        "datasource": {
...
def test_status_report_prints_section_headers(tmp_path, evidence_output_dir, capsys):
    report = EvidenceReport(subject="REP-003: status_report produces TRAINING, TUNING, and MODEL TESTING section headers")
    ...
    report.info("status_report output contains TRAINING, TUNING, and MODEL TESTING section headers", "REP-003")
...
def test_status_report_prints_training_section(tmp_path, evidence_output_dir, capsys):
    report = EvidenceReport(subject="REP-004: status_report prints training metrics (final_loss, learning_rate)")
    ...
    report.info("status_report printed TRAINING section with final_loss and learning_rate", "REP-004")
...
def test_status_report_prints_tuning_section(tmp_path, evidence_output_dir, capsys):
    report = EvidenceReport(subject="REP-004: status_report prints tuning metrics (num_trials, best_val_loss)")
...
def test_status_report_prints_model_testing_section(tmp_path, evidence_output_dir, capsys):
    report = EvidenceReport(subject="REP-004: status_report prints model testing metrics (dice, iou, precision, recall)")
    ...
    report.info("status_report printed MODEL TESTING metrics: dice, iou, precision, recall, num_test_patients", "REP-004")
