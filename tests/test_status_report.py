...
def test_status_report_prints_training_section(tmp_path, evidence_output_dir, capsys):
    report = EvidenceReport(subject="status_report prints training metrics")
    ...
    checks = [
        ("TRAINING", "TRAINING section header"),
        ("0.0847", "final_loss value"),
        ("0.001", "learning_rate value"),
    ]
...
def test_status_report_prints_tuning_section(tmp_path, evidence_output_dir, capsys):
    report = EvidenceReport(subject="status_report prints hyperparameter tuning metrics")
    ...
    checks = [
        ("TUNING", "TUNING section header"),
        ("4", "num_trials"),
        ("0.0901", "best_val_loss"),
    ]
    for token, desc in checks:
        if token not in out:
            report.error(f"Expected '{token}' ({desc}) not found in output", "REP-004")
...
def test_status_report_prints_model_testing_section(tmp_path, evidence_output_dir, capsys):
    report = EvidenceReport(subject="status_report prints model testing metrics")
